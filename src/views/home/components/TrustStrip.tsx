...
import { Lock, Fingerprint, FileCheck } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <path
        ...
        fill="white"
      />
      <path d="M194.9 34.1001H169.8V121.6H194.9V34.1001Z" fill="white" />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
...
const Divider = () => (
  <span className="hidden sm:block w-px h-3 bg-white/15 flex-shrink-0" aria-hidden="true" />
);
...
function TrustItem({ icon, label }: TrustItemProps) {
  return (
    <div className="flex items-center gap-2 text-white opacity-45 hover:opacity-80 transition-opacity duration-300 cursor-default select-none">
...
export function TrustStrip() {
  return (
    <div
      className="flex flex-col sm:flex-row flex-wrap items-center justify-center gap-y-4 gap-x-8"
      role="list"
